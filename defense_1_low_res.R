...
defense_low_res$beta <- rbind(rep(6,5),
                              rep(6,5),
                              rep(6,5),
                              rep(6,5),
                              rep(6,5))

fit <- stan("models/defense_1.stan", data = defense_low_res, chains = 4, iter = 1e3, cores = 4)
saveRDS(list(fit = fit, data = defense_low_res), paste0("results/defense_1_", evt_id, "_low_res.RDS"))
...
}, video.name = paste0("media/defense_1_", evt_id, "_low_res",".mp4"))
