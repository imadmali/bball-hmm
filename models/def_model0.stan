...
model {
  ...
  real gamma[N, K];
  for (k in 1:K)
    gamma[1, k] = normal_lpdf(d[1] |  o1[1]*lambda[1] + h[1]*lambda[2] + b[1]*lambda[3], cc_sd)
      + normal_lpdf(d[1] |  o2[1]*lambda[1] + h[1]*lambda[2] + b[1]*lambda[3], cc_sd);
  for (t in 2:N) {
    for (k in 1:K) {
      for (j in 1:K)
...
generated quantities {
  ...
  real log_p_y_star;
  {
    ...
    for (k in 1:K)
      best_logp[1, k] = normal_lpdf(d[1] |  o1[1]*lambda[1] + h[1]*lambda[2] + b[1]*lambda[3], cc_sd)
        + normal_lpdf(d[1] |  o2[1]*lambda[1] + h[1]*lambda[2] + b[1]*lambda[3], cc_sd);
    for (t in 2:N) {
      for (k in 1:K) {
